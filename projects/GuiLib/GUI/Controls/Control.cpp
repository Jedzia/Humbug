...
void CControl::Draw() {
    ...
    // the coordinates have to be relative and recursive paint must be used by the control parent to
    // draw.
...
bool CControl::OnEvent(SDL_Event* pEvent) {
    ...
    {
        return OnMouseMove(pEvent->motion.x, pEvent->motion.y, pEvent->motion.xrel, pEvent->motion.yrel, (pEvent->motion.state & SDL_BUTTON(
                                                                                                                  SDL_BUTTON_LEFT)) != 0,
                (pEvent->motion.state & SDL_BUTTON(SDL_BUTTON_RIGHT)) != 0, (pEvent->motion.state & SDL_BUTTON(
                                                                                     SDL_BUTTON_MIDDLE)) !=
                0);
...
bool CControl::FilterEvent(SDL_Event* pEvent) {
    ...
    if(GetMouseFocus()) {
        ...
        case SDL_MOUSEMOTION:        //mouse motion
        {
            return GetMouseFocus()->OnMouseMove(pEvent->motion.x, pEvent->motion.y, pEvent->motion.xrel, pEvent->motion.yrel,
                    (pEvent->motion.state & SDL_BUTTON(SDL_BUTTON_LEFT)) != 0, (pEvent->motion.state & SDL_BUTTON(SDL_BUTTON_RIGHT)) != 0,
